export const en = {
  meta: {
    title: "Evento",
    description: "Book venues, vendors, and event services worldwide.",
  },
  brand: "Evento",
  nav: {
    home: "Home",
    vendors: "Vendors",
    categories: "Categories",
    offers: "Offers",
    reviews: "Reviews",
    dashboard: "Dashboard",
    bookings: "Bookings",
    profile: "Profile",
    accountMenu: "Account menu",
    admin: "Admin",
    becomeVendor: "List your business",
    login: "Sign in",
    menu: "Menu",
  },
  language: {
    ar: "العربية",
    en: "English",
    switchTo: "Language",
  },
  pwa: {
    install: "Install Evento",
    dismiss: "Not now",
  },
  home: {
    heroEyebrow: "The worldwide event marketplace",
    heroTitle: "Find it. Compare it. Book it.",
    heroSubtitle:
      "Venues, photographers, catering, and more, search by city and date, then book trusted vendors worldwide.",
    searchCategory: "Categories",
    searchCategoriesPlaceholder: "Any specialty",
    searchCategoriesHint: "Select one or more",
    searchCategoriesFilter: "Search specialties",
    searchCategoriesEmpty: "No matching specialties",
    searchSelected: "selected",
    searchClearCategories: "Clear selection",
    searchRemoveCategory: "Remove",
    searchAnyCity: "Any city",
    searchCurrentLocation: "Current location",
    searchLocating: "Finding your location…",
    searchLocationDenied: "Could not read your location. Allow access and try again.",
    searchCity: "City",
    searchDate: "Date",
    searchCta: "Search",
    browseVendors: "Browse vendors",
    categoriesHeading: "Every detail of your event",
    categoriesSubtitle:
      "From the venue to the last detail, compare vendors and book by date.",
    viewAll: "View all",
    seeAll: "See all",
    prevCategories: "Previous categories",
    nextCategories: "Next categories",
    featuredHeading: "Featured vendors",
    featuredSubtitle: "Verified, reviewed, and ready to book this season.",
    featuredEmpty: "Featured vendors will appear here after accounts are approved.",
    featuredEmptyHint: "Check back soon, or browse all categories in the meantime.",
    offersHeading: "Offers",
    offersSubtitle: "Limited deals from verified vendors, book before they expire.",
    offerDiscount: "{percent}% off",
    offerUntil: "Until {date}",
    socialEyebrow: "Why families book on Evento",
    socialHeading: "Chosen with family. Booked with confidence.",
    socialSubtitle:
      "Verified vendors, real ratings, and coverage worldwide, so the booking feels as sure as the gathering.",
    socialCouplesValue: "12,400+",
    socialCouplesLabel: "events planned with Evento",
    socialRatingValue: "4.9",
    socialRatingLabel: "average vendor rating",
    socialVendorsValue: "860+",
    socialVendorsLabel: "verified vendors",
    socialCitiesValue: "18",
    socialCitiesLabel: "cities worldwide",
    reviewsHeading: "What clients say",
    reviewsSubtitle: "Notes from events booked on Evento, each one tagged by specialty.",
    reviewCategory: "Category",
  },
  footer: {
    tagline: "Book venues and vendors for celebrations worldwide.",
    categories: "Categories",
    explore: "Explore",
    company: "Company",
    about: "About Evento",
    contact: "Contact",
    help: "Help center",
    forVendors: "For vendors",
    becomeVendor: "Join as a vendor",
    cities: "Popular cities",
    privacy: "Privacy",
    terms: "Terms",
    copyright: "Evento. All rights reserved.",
  },
  categories: {
    venues: {
      title: "Venues",
      description: "Wedding halls and event spaces booked by date.",
    },
    bachelorette: {
      title: "Bachelorette",
      description: "Parties, activities, and nights out for the bride and her friends.",
    },
    "beauty-nails": {
      title: "Beauty & Nails",
      description: "Bridal nails, spa, and beauty treatments before the day.",
    },
    catering: {
      title: "Catering",
      description: "Menus priced per guest, with a minimum headcount.",
    },
    dj: {
      title: "DJ",
      description: "Music, lighting, and a dance floor that keeps the night going.",
    },
    entertainment: {
      title: "Entertainment",
      description: "Artists, photobooths, and live acts for the celebration.",
    },
    "favours-products": {
      title: "Favours & Products",
      description: "Guest gifts, souvenirs, and custom event products.",
    },
    florist: {
      title: "Florist",
      description: "Bouquets, centerpieces, and floral design for the day.",
    },
    hairdresser: {
      title: "Hairdresser",
      description: "Bridal and party hairstyling, with optional trials.",
    },
    honeymoon: {
      title: "Honeymoon",
      description: "Travel packages and getaways after the wedding.",
    },
    "makeup-artists": {
      title: "Makeup Artist",
      description: "Bridal and event makeup, with optional trials.",
    },
    "media-coverage": {
      title: "Media Coverage",
      description: "Press, social, and event media coverage for the day.",
    },
    officiant: {
      title: "Officiant",
      description: "The person who leads the ceremony and legal vows.",
    },
    photographers: {
      title: "Photography",
      description: "Photo coverage booked by package.",
    },
    "room-decoration": {
      title: "Room Decoration",
      description: "Styling and décor for halls, suites, and reception rooms.",
    },
    transportation: {
      title: "Transportation",
      description: "Wedding cars, guest transfers, and arrival-day logistics.",
    },
    "veil-designer": {
      title: "Veil designer",
      description: "Custom veils and headpieces for the bride.",
    },
    videography: {
      title: "Videography",
      description: "Films and highlight reels booked by package.",
    },
    "wedding-cake": {
      title: "Wedding Cake",
      description: "Cakes and dessert tables designed for the reception.",
    },
    planners: {
      title: "Wedding planner",
      description: "Full planning packages for your event.",
    },
    "photo-locations": {
      title: "Photo Session Locations",
      description: "Indoor and outdoor locations booked by the hour.",
    },
  },
  cities: {
    cairo: "Cairo",
    giza: "Giza",
    alexandria: "Alexandria",
    "north-coast": "North Coast",
    luxor: "Luxor",
    aswan: "Aswan",
  },
  vendors: {
    title: "Browse vendors",
    subtitle: "Compare trusted vendors and book by date.",
    filters: "Filters",
    category: "Category",
    city: "City",
    price: "Price",
    rating: "Rating",
    allCategories: "All categories",
    allCategoriesSubtitle: "Browse every specialty and open the vendors in each one.",
    empty: "Vendors in this category will appear here after admin approval.",
  },
  offers: {
    title: "Offers",
    subtitle: "Browse every current deal and filter by specialty.",
    allCategories: "All categories",
    filterCategory: "Filter by category",
    emptyTitle: "No offers in this category",
    emptyHint: "Try another specialty, new deals are added through the season.",
  },
  vendor: {
    gallery: "Gallery",
    packages: "Packages & pricing",
    availability: "Availability",
    reviews: "Reviews",
    book: "Request a booking",
    bookNow: "Book now",
    fromPrice: "From",
    away: "away",
    perEvent: "per event",
    perDay: "per day",
    verified: "Verified",
    guests: "guests",
    available: "Available",
    booked: "Booked",
    selected: "Selected",
    description: "About",
    location: "Location",
    startingFrom: "Starting from",
    reviewsCount: "reviews",
    services: "What’s included",
    zoom: "View photo",
    closeGallery: "Close gallery",
    prevPhoto: "Previous photo",
    nextPhoto: "Next photo",
    sizes: "Size",
    fittingDate: "Fitting date",
    pickupDate: "Pickup",
    returnDate: "Return",
    selectSize: "Select a size",
    angles: {
      front: "Front",
      back: "Back",
      side: "Side",
      detail: "Details",
    },
    noReviews: "No reviews yet",
    noReviewsHint: "Be the first to share how this vendor made your day feel.",
    weekdays: {
      sat: "Sat",
      sun: "Sun",
      mon: "Mon",
      tue: "Tue",
      wed: "Wed",
      thu: "Thu",
      fri: "Fri",
    },
  },
  empty: {
    title: "Nothing here yet",
    searchTitle: "No matching vendors",
    searchHint:
      "Try another city, date, or category, new vendors join Evento every week.",
    bookingsTitle: "No bookings yet",
    bookingsHint: "When you request a booking, it will appear here with its status.",
    vendorBookingsTitle: "No booking requests yet",
    vendorBookingsHint:
      "When a customer requests a date, it will show here so you can accept or cancel.",
  },
  booking: {
    title: "Complete your booking",
    subtitle: "Choose a date, package, and how you would like to pay.",
    eventDate: "Event date",
    eventType: "Event type",
    eventTypes: {
      wedding: "Wedding",
      engagement: "Engagement",
      birthday: "Birthday",
      general: "Other event",
    },
    package: "Package",
    startingPackage: "Standard booking",
    notes: "Notes for the vendor",
    notesPlaceholder: "Guest count, timing, or anything the vendor should know.",
    payment: "Payment method",
    stepBooking: "Booking",
    stepPayment: "Payment",
    back: "Back",
    cardName: "Name on card",
    cardNumber: "Card number",
    cardExpiry: "Expiry",
    cardCvc: "CVC",
    cardNumberPlaceholder: "ACCT-000015",
    cardExpiryPlaceholder: "MM/YY",
    walletProvider: "Wallet",
    walletPhone: "Wallet mobile number",
    walletPhonePlaceholder: "01xxxxxxxxx",
    wallets: {
      vodafone: "Vodafone Cash",
      orange: "Orange Cash",
      etisalat: "Etisalat Cash",
      instapay: "InstaPay",
    },
    payAmount: "Pay {amount}",
    paymentInvalid: "Check your payment details and try again.",
    methods: {
      card: {
        title: "Card",
        hint: "Enter your card number, expiry, and CVC to pay.",
      },
      wallet: {
        title: "Mobile wallet",
        hint: "Enter the mobile number linked to your wallet.",
      },
      venue: {
        title: "Pay the vendor",
        hint: "No card needed. Confirm the date, then pay the vendor later.",
      },
    },
    summary: "Booking summary",
    total: "Total",
    submit: "Request booking",
    submitPay: "Continue",
    successTitle: "Request sent",
    successBody: "The vendor will review your date. We will email you when it is confirmed.",
    successPayBody: "We received your payment details and sent the booking request to the vendor.",
    viewBookings: "View my bookings",
    backToVendor: "Back to listing",
    invalid: "Please complete the required fields.",
    dateUnavailable: "That date is not available.",
    failed: "Could not send this booking. Try again.",
    secure: "Your request is private to you and the vendor.",
    notice: "Dates open from {date}, with 3 days of notice, same as search.",
    requestsTitle: "Booking requests",
    customerSubtitle: "Track the status of each request you sent to a vendor.",
    vendorSubtitle: "Accept a request to confirm the date, or cancel it if you cannot take it.",
    customerName: "Customer",
    unknownCustomer: "Customer",
    unknownVendor: "Vendor",
    statusLabel: "Status",
    actions: "Actions",
    noActions: "No actions",
    accept: "Accept",
    cancelRequest: "Cancel",
    acceptFailed: "Could not accept this booking. Try again.",
    cancelFailed: "Could not cancel this booking. Try again.",
    pendingCount: "{count} pending requests",
    statuses: {
      pending: "Pending",
      confirmed: "Confirmed",
      cancelled: "Cancelled",
    },
  },
  dashboard: {
    overview: "Dashboard",
    bookings: "Bookings",
    profile: "Profile",
    vendorServices: "Services",
    vendorAvailability: "Availability",
    vendorHome: "Vendor home",
    customerIntro: "Track your bookings and account details.",
    vendorIntro: "Manage your offerings, calendar, and incoming bookings.",
    comingSoon: "This dashboard section will be connected in a later step.",
    becomeVendorTitle: "Become a vendor",
    becomeVendorBody:
      "Register your business from this account. You will not create a second email or password.",
    becomeVendorCta: "Start vendor application",
  },
  profile: {
    title: "Profile",
    subtitle: "Your photo and account details on Evento.",
    email: "Email",
    phone: "Phone",
    role: "Account type",
    memberSince: "Member since",
    noPhone: "Not added yet",
    photoReady: "Profile photo, this is the image others see on your account.",
    photoMissing: "No photo yet. Initials are shown until you add one.",
    uploadPhoto: "Upload a photo",
    savePhoto: "Save photo",
    photoSaved: "Profile photo saved.",
    unavailable: "We could not load your profile. Sign in again and try once more.",
    roles: {
      customer: "Customer",
      vendor: "Vendor",
      admin: "Admin",
    },
  },
  admin: {
    overview: "Admin",
    vendors: "Vendors",
    bookings: "Bookings",
    intro: "Review vendors, bookings, and platform stats.",
    comingSoon: "Admin tools will be connected after authentication is in place.",
    approve: "Approve",
    status: "Status",
    pending: "Pending",
    approved: "Approved",
    rejected: "Rejected",
    suspended: "Suspended",
    showApplication: "Show application",
    applicationDetails: "Review this vendor's application before approving.",
    applicationInfo: "Listing details",
    applicationMedia: "Photos",
    openInMaps: "Open in Google Maps",
    noLocation: "No map location provided",
    close: "Close",
    actions: "Actions",
    approveFailed: "Could not approve this vendor. Try again.",
    ban: "Ban",
    deleteConfirmTitle: "Delete {name}?",
    deleteConfirmBody:
      "This permanently deletes their listing, user account, and email from the database. This cannot be undone.",
    deleteConfirm: "Delete",
    cancel: "Cancel",
    deleteFailed: "Could not delete this vendor. Try again.",
    pendingApplications: "{count} pending applications",
  },
  common: {
    comingSoon: "Coming soon",
    loading: "Loading",
    submit: "Continue",
    uploading: "Uploading…",
    uploadingPercent: "{percent}%",
    uploadFailed: "Could not upload this file. Try again.",
    choosePhoto: "Choose photo",
    addPhotos: "Add photos or videos",
    changePhoto: "Change photo",
    photosCount: "{count} / {max}",
    removePhoto: "Remove",
    imageTooLarge: "Each image must be 2.5 MB or smaller.",
    videoTooLarge: "Each video must be 10 MB or smaller.",
    tooManyImages: "You can add up to 10 gallery photos.",
    tooManyVideos: "You can add up to 3 gallery videos.",
  },
  auth: {
    signInTitle: "Welcome back",
    signInSubtitle: "Sign in to manage bookings and your Evento account.",
    signUpTitle: "Create your account",
    signUpSubtitle: "Join as a couple planning an event, or as a vendor.",
    email: "Email",
    password: "Password",
    fullName: "Full name",
    customer: "I'm a customer",
    vendor: "I'm a vendor",
    submitSignIn: "Sign in",
    submitSignUp: "Create account",
    noAccount: "New to Evento?",
    hasAccount: "Already have an account?",
    signUpLink: "Create an account",
    signInLink: "Sign in",
    forgotPassword: "Forgot password?",
    forgotTitle: "Reset your password",
    forgotSubtitle: "We'll send a reset link to your email.",
    sendReset: "Send reset link",
    resetSent: "If an account exists, a reset email is on its way.",
    checkEmailTitle: "Confirm your email",
    checkEmailBody:
      "We sent a confirmation link to your inbox. You can sign in after you open it.",
    backToSignIn: "Back to sign in",
    updatePasswordTitle: "Choose a new password",
    updatePasswordSubmit: "Save password",
    signOut: "Sign out",
    invalidCredentials: "Email or password is incorrect.",
    emailNotConfirmed: "Confirm your email before signing in.",
    networkError: "Could not reach the sign-in service. Try again.",
    genericError: "Something went wrong. Please try again.",
    emailTaken: "This email is already registered.",
    emailTakenOr: "instead, or",
    emailTakenForgot: "if you forgot it.",
    logIn: "Log in",
    resetYourPassword: "reset your password",
    emailTakenVendor:
      "You already have an account with this email. Log in, then use “Become a Vendor” from your dashboard to register your business.",
    emailTakenVendorLead: "You already have an account with this email.",
    emailTakenVendorRest:
      ", then use “Become a Vendor” from your dashboard to register your business.",
  },
  onboarding: {
    title: "Tell us about your business",
    stepBusiness: "Business",
    stepDetails: "All fields are required so we can review your listing.",
    stepReview: "Review",
    businessName: "Business name",
    category: "Category",
    city: "City",
    address: "Address",
    locationLink: "Google Maps location link",
    locationHint: "Open Google Maps, drop a pin, then paste the share link.",
    startingPrice: "Starting price (EGP)",
    description: "Description",
    coverImage: "Cover photo",
    galleryImages: "Gallery photos and videos",
    coverHint: "One image, up to 2.5 MB.",
    galleryHint: "Up to 10 photos (2.5 MB each) and 3 videos (10 MB each).",
    required: "This field is required.",
    invalid: "Please complete every field with a valid value.",
    next: "Next",
    back: "Back",
    submit: "Submit for review",
    pendingTitle: "Your account is under review",
    pendingBody:
      "An Evento admin will approve your listing before it appears to customers. You can browse the platform in the meantime.",
    rejectedTitle: "Your listing needs changes",
    rejectedBody: "Please update your details and wait for a new review.",
    approvedBody: "Your listing is live. You can manage packages and availability.",
  },
} as const;

type StringLeaves<T> = {
  [K in keyof T]: T[K] extends string ? string : StringLeaves<T[K]>;
};

export type Dictionary = StringLeaves<typeof en>;
